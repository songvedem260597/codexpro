...
import { createWorkerUpdateBroadcaster } from "./worker-update-broadcaster.mjs";
import { createProjectDiscovery } from "./project-discovery.mjs";
...
const {
  ...
  localMcpTool
} = createManagerMcpClient({
  managerVersion: MANAGER_VERSION,
  diagnostic,
  diagnosticAllowed,
  causalTelemetry: mcpCausalTelemetry,
  emitCausalTelemetry: emitMcpCausalTelemetry
});
...
const {
  ...
  codexProHome,
  readTaskConfig: async () => {
    const task = await scheduledTask();
    return parseTaskArguments(task.arguments);
  },
  readManagerSettings,
  diagnostic,
  allAllowedWorkspaces: ALL_ALLOWED_WORKSPACES,
  pathInside
});
...
diagnosticIpcHandle("codexpro:send-profile-request", {
  ...
  resultDetails: (result) => {
    ...
    return {
      ...
      send_control_wait_ms: Number(value?.send_control_wait_ms) || 0,
      send_control_disabled_duration_ms: Number(value?.send_control_disabled_duration_ms) || 0,
      form_inert_duration_ms: Number(value?.form_inert_duration_ms) || 0,
      first_send_ready_at_ms: Number(value?.first_send_ready_at_ms) || 0,
      send_control_readiness_timeline: Array.isArray(value?.send_control_readiness_timeline) ? value.send_control_readiness_timeline : [],
