...
const launcher = [
  await fs.readFile(new URL("../../scripts/codexpro.mjs", import.meta.url), "utf8"),
  await fs.readFile(new URL("../../scripts/cli-process-runtime.mjs", import.meta.url), "utf8")
].join("\n");
const managerMain = await fs.readFile(new URL("../electron/main.mjs", import.meta.url), "utf8");
assert.match(launcher, /record\(process(?:Runtime)?\.stdout, 'stdout', chunk\)/, "cloudflared stdout must retain its stream name and actual output chunk");
assert.match(launcher, /record\(process(?:Runtime)?\.stderr, 'stderr', chunk\)/, "cloudflared stderr must retain its stream name and actual output chunk");
