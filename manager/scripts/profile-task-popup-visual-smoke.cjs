...
const taskItem = ({ status, label, title, progress, current = false, reason = "" }) => `
<article class="profile-task-item is-${status} ${current ? "is-current" : ""}">
  ...
  <div class="profile-task-item-foot"><span>codexpro-source</span><button class="button primary profile-task-resume">Tiếp tục task</button></div>
</article>`;
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const metrics = await win.webContents.executeJavaScript(`(() => {
      ...
      return { modal: {left:modal.left,right:modal.right,top:modal.top,bottom:modal.bottom,width:modal.width,height:modal.height}, viewport:{w:innerWidth,h:innerHeight}, taskTop:taskButton.top, chatTop:chatButton.top, list:{scrollHeight:list.scrollHeight,clientHeight:list.clientHeight,itemCount:list.querySelectorAll('.profile-task-item').length,completedCount:list.querySelectorAll('.profile-task-item.is-completed').length}, resumeDisabled:resume.disabled };
    })()`);
    ...
    if (metrics.list.itemCount !== 2 || metrics.list.completedCount !== 0) throw new Error(`popup should show only failed/unfinished tasks: ${JSON.stringify(metrics)}`);
