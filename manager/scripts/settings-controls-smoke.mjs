...
assert.match(main, /import \{ SettingsView \} from "\.\/features\/settings\/settings-view\.jsx";/, "main.jsx must consume the extracted SettingsView seam");
assert.match(settingsView, /import \{ SettingsDropdown, SettingsToggle \} from "\.\.\/\.\.\/components\/settings-controls\.jsx";/, "SettingsView must consume the extracted settings controls module");
for (const component of ["SettingsDropdown", "SettingsToggle"]) {
  ...
  assert.doesNotMatch(settingsView, new RegExp(`function ${component}\\(`), `${component} implementation must stay out of SettingsView`);
...
assert.equal((settingsView.match(/<SettingsDropdown\b/g) || []).length, 6, "settings page must keep all six dropdown usages");
assert.equal((settingsView.match(/<SettingsToggle\b/g) || []).length, 1, "settings page must keep the chat selector toggle");
