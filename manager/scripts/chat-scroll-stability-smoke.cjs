...
app.whenReady().then(async () => {
  ...
  try {
    ...
    if (result.order.join("|") !== "old assistant one|old user|old assistant two|new user|canonical response replaced without moving the active user turn") throw new Error(`Chat order changed: ${JSON.stringify(result.order)}`);
