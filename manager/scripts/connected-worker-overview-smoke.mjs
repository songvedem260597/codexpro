...
assert.match(styles, /\.page-overview \.header-server-actions \{ width: auto;[\s\S]*?\.profile-count \{ min-height: 42px;[\s\S]*?font-size: 12px;[\s\S]*?\.profile-summary-icon \{ width: 19px; height: 19px;[\s\S]*?\.profile-summary-icon svg \{ width: 19px; height: 19px; stroke-width: 2;/, "overview worker totals must keep larger, readable status icons without becoming oversized");
