...
assert.match(source, /profile-summary-states[\s\S]*?profile-summary-updates[\s\S]*?<span>cập nhật<\/span>/, "worker updates must stay grouped beside the state totals in the same compact row");
...
assert.match(styles, /\.page-overview > header \{ display: grid; grid-template-columns: minmax\(260px, \.65fr\) minmax\(0, 1\.35fr\);[\s\S]*?\.profile-count \{ min-height: 42px; display: grid; grid-template-columns: minmax\(0, 1fr\) auto;[\s\S]*?\.profile-summary-states \{[^}]*display: flex;[^}]*flex-wrap: nowrap;[\s\S]*?\.profile-summary-icon \{ width: 19px; height: 19px;[\s\S]*?\.profile-summary-updates \{[^}]*padding-left: 7px; border-left: 1px solid #29313e;/, "overview worker totals and update indicator must stay on one desktop row with readable icons");
