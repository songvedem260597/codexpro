...
import {
  ...
  browserProfileRetentionState,
  mergeBrowserExtensionStreamBatch,
  normalizeRecentConversations,
  PROFILE_REGISTRY_VERSION,
  PROFILE_RETENTION_MS,
  PROFILE_TTL_MS,
  type BrowserExtensionStreamUpdate,
  type BrowserProfilePersistenceRecord
} from "./browserExtensionProfileState.js";

export { browserProfilePersistenceSnapshot, browserProfileRetentionState, mergeBrowserExtensionStreamBatch };
export type { BrowserExtensionStreamUpdate, BrowserProfilePersistenceRecord };
...
function loadBrowserProfileRegistry(state: BridgeState, now = Date.now()): void {
  try {
    ...
    // A missing or corrupt snapshot behaves like a clean first run.
  }
}
...
export function listBrowserExtensionProfiles(): ExtensionProfileSummary[] {
  ...
  return visibleProfiles
    .map((profile) => {
      ...
      active: browserProfileRetentionState(profile, now).connected && state.activeProfileId === profile.id,
      connected: browserProfileRetentionState(profile, now).connected,
...
export function setBrowserExtensionProfileWorkspace(profileId: string, root: string): void {
  ...
  if (singleton) scheduleBrowserProfileRegistryPersistence(singleton);
