...
export function createCodexProServer(config: CodexProConfig, context: CodexProServerContext = {}): McpServer {
  ...
  if (!requireRepoTask) registerCodexTool(
    ...
    "prepare_repo_task",
    {
      ...
      inputSchema: {
        profile_id: z.string().regex(/^[A-Za-z0-9._-]{1,160}$/).describe("Profile-bound worker id receiving the Manager request, including Chrome or API workers."),
  ...
  registerCodexTool(
    ...
    async (args) => {
      ...
      if (requireRepoTask) {
        if (!gateProfileId) {
          throw new CodexProError(
            "REPO_TASK_PROFILE_REQUIRED: The MCP connector is not bound to a Chrome profile.",
