...
- Added CodexPro-native execute/audit loops: `loop-handoff` can now run without an external `--review-command`, preserve the original task as an immutable audit target, invoke the independent read-only `codexpro-auditor`, convert actionable FAIL findings into the next remediation plan, and only complete after a verified PASS that is not contradicted by executor/test failures. Added deterministic two-iteration regression coverage and `npm run audit:live` for a real OpenCode audit session.
