...
const MAC_MAX_CHATGPT_TABS = 1;
const CHAT_TAB_CLEANUP_INTERVAL_MS = 30 * 1000;
const CHAT_TAB_HEALTH_TIMEOUT_MS = 1200;
const CHAT_TAB_HEALTH_FAILURES_TO_CLOSE = 2;
const PROFILE_LIFECYCLE_STORAGE_KEY = 'codexproProfileLifecycleV1';
const PROFILE_LIFECYCLE_HISTORY_STORAGE_KEY = 'codexproProfileLifecycleHistoryV1';
const PROFILE_LIFECYCLE_HISTORY_LIMIT = 20;
...
let latestProfileLifecycleEvent = null;
let profileLifecycleHistory = [];
let profileLifecycleWriteTail = Promise.resolve();
...
function safeTabAuditUrl(value) {
  try{const url=new URL(String(value||''));if(url.origin!=='https://chatgpt.com')return '';return `${url.origin}${url.pathname}`;}catch{return '';}
}

function mergeProfileLifecycleEvents(...groups) {
  const merged=[];
  const seen=new Set();
  for(const event of groups.flat()){
    if(!event||typeof event!=='object'||Array.isArray(event))continue;
    const key=String(event.event_id||`${event.at||''}|${event.type||''}|${event.reason||''}|${event.tab_id||''}|${event.window_id||''}`);
    if(seen.has(key))continue;
    seen.add(key);
    merged.push(event);
  }
  return merged.slice(-PROFILE_LIFECYCLE_HISTORY_LIMIT);
}

function recordProfileLifecycleEvent(event={}) {
  const value={event_id:crypto.randomUUID(),at:new Date().toISOString(),...event};
  latestProfileLifecycleEvent=value;
  profileLifecycleHistory=mergeProfileLifecycleEvents(profileLifecycleHistory,value);
  profileLifecycleWriteTail=profileLifecycleWriteTail.catch(()=>{}).then(async()=>{
    const stored=await chrome.storage.local.get([PROFILE_LIFECYCLE_HISTORY_STORAGE_KEY]);
    profileLifecycleHistory=mergeProfileLifecycleEvents(stored[PROFILE_LIFECYCLE_HISTORY_STORAGE_KEY]||[],profileLifecycleHistory);
    await chrome.storage.local.set({
      [PROFILE_LIFECYCLE_STORAGE_KEY]:value,
      [PROFILE_LIFECYCLE_HISTORY_STORAGE_KEY]:profileLifecycleHistory
    });
  });
  scheduleRealtimeProfilePush(0);
  return profileLifecycleWriteTail;
}

async function removeTabWithReason(tabId,reason,details={}) {
  let tab=null;
  try{tab=await chrome.tabs.get(tabId);}catch{}
  tabRemovalIntents.set(tabId,{reason:String(reason||'codexpro_internal'),window_id:Number.isInteger(tab?.windowId)?tab.windowId:null,at:Date.now(),...details});
  await recordProfileLifecycleEvent({type:'tab_close_requested',reason:String(reason||'codexpro_internal'),tab_id:tabId,window_id:Number.isInteger(tab?.windowId)?tab.windowId:null,url:String(tab?.url||''),details}).catch(()=>{});
  try{await chrome.tabs.remove(tabId);}
  catch(error){tabRemovalIntents.delete(tabId);throw error;}
}

async function chatGptTabLimit() {
  if(Number.isInteger(chatTabLimitCache)&&chatTabLimitCache>0)return chatTabLimitCache;
  try{
    const platform=await chrome.runtime.getPlatformInfo();
    chatTabLimitCache=platform?.os==='mac'?MAC_MAX_CHATGPT_TABS:MAX_CHATGPT_TABS;
  }catch{chatTabLimitCache=MAX_CHATGPT_TABS;}
...
function planChatTabCleanup(tabs,options={}) {
  ...
  const protectedTab=tab=>Boolean((tab.active&&!allowActiveIdle)||tab.pinned||tab.audible||tab.status==='loading'||tab.busy||tab.settling||tab.pending);
...
async function cleanupChatGptTabs(tabSummaries,recentConversations,options={}) {
  ...
  const platformLimit=await chatGptTabLimit();
  const requestedMaxTabs=Number(options.maxTabs);
  const maxTabs=Number.isFinite(requestedMaxTabs)?Math.max(0,Math.floor(requestedMaxTabs)):platformLimit;
  const singleTabProfile=platformLimit===MAC_MAX_CHATGPT_TABS;
  const allowActiveIdle=options.allowActiveIdle===true||singleTabProfile;
    ...
    try{
      ...
      if(singleTabProfile&&plan.reasons[tabId]==='codexpro_unreachable'){
        ...
        if(remaining.length===1&&remaining[0].id===tabId){
          // Closing the last tab can terminate the profile window and this worker.
          // Keep the original intact if creation fails; never create after closing it.
          if(maxTabs<1)continue;
          try{
            const replacement=await chrome.tabs.create({url:String(current.url),active:Boolean(current.active),...(Number.isInteger(current.windowId)?{windowId:current.windowId}:{})});
            healthReplacement=await chrome.tabs.get(replacement.id);
          }catch(error){
            await recordProfileLifecycleEvent({type:'health_replacement_failed',reason:'preserve_last_mac_tab',tab_id:tabId,error:String(error?.message||error).slice(0,300)}).catch(()=>{});
            continue;
          }
        }
      }
      await removeTabWithReason(tabId,`chat_cleanup:${plan.reasons[tabId]||'tab_limit'}`,{url:String(current.url||'')});
      closed.push({tab_id:tabId,reason:plan.reasons[tabId]||'tab_limit',url:String(current.url||''),active:Boolean(current.active),window_id:Number.isInteger(current.windowId)?current.windowId:null});
    }catch{}
  }
  if(closed.length){
    await chrome.storage.local.set({codexproChatTabCleanup:{at:new Date().toISOString(),max_tabs:maxTabs,closed,replacement_tab_id:healthReplacement?.id||null}}).catch(()=>{});
    recentConversationCache={at:0,items:[]};
    scheduleRealtimeProfilePush(0);
  }
  return {...plan,closed,closed_count:closed.length,replacement_tab_id:healthReplacement?.id||null};
}

function enforceSingleChatTabSoon() {
  singleChatTabEnforcementTail=singleChatTabEnforcementTail.catch(()=>{}).then(async()=>{
    if(await chatGptTabLimit()!==MAC_MAX_CHATGPT_TABS)return;
    const [tabs,recentConversations]=await Promise.all([tabList(),recentConversationList(1)]);
    await cleanupChatGptTabs(tabs,recentConversations,{force:true,maxTabs:MAC_MAX_CHATGPT_TABS,allowActiveIdle:true});
  });
  return singleChatTabEnforcementTail;
}
...
async function focusNewChatGptTab(tab,source='new_chat_tab') {
  ...
  try{await recordProfileLifecycleEvent({type:'chat_tab_focused',reason:String(source||'new_chat_tab'),tab_id:focusedTab.id,window_id:windowId,url:String(focusedTab.url||'')});}catch{}
  return {tab:focusedTab,window_info:windowInfo};
}

async function createChatGptTab(createArgs={},source='chat_tab_create') {
  ...
  return await serializeChatGptTabCreation(async()=>{
    ...
    if(tabLimit===MAC_MAX_CHATGPT_TABS&&current.length){
      const reusable=current.find(tab=>tab.active)||current[0];
      const summaries=await tabList();
      const summary=summaries.find(tab=>tab.id===reusable.id)||{};
      const conversationId=conversationIdFromUrl(reusable.url);
      const [networkState,domActivity]=await Promise.all([
      ...
      let updated=await chrome.tabs.update(reusable.id,{url,active:true});
      updated=(await focusNewChatGptTab(updated,source)).tab;
      recordProfileLifecycleEvent({type:'chat_tab_reused',reason:'mac_single_tab_limit',tab_id:reusable.id,window_id:reusable.windowId,previous_url:previousUrl,next_url:url});
      return {...updated,codexpro_reused:true,codexpro_previous_url:previousUrl};
    }
    if(current.length>=tabLimit){
      const summaries=await tabList();
      const recentConversations=await recentConversationList(Math.max(1,tabLimit));
      await cleanupChatGptTabs(summaries,recentConversations,{force:true,maxTabs:tabLimit-1,allowActiveIdle:tabLimit===MAC_MAX_CHATGPT_TABS});
      current=(await chrome.tabs.query({url:['https://chatgpt.com/*']})).filter(tab=>Number.isInteger(tab.id));
    }
    if(current.length>=tabLimit)throw new Error(`CHAT_TAB_LIMIT_REACHED: Worker chỉ cho phép tối đa ${tabLimit} tab ChatGPT trên profile này; tab hiện tại đang hoạt động hoặc được bảo vệ.`);
    let created=await chrome.tabs.create({...createArgs,active:true});
    created=(await focusNewChatGptTab(created,source)).tab;
    return created;
  });
}
...
function isAttachmentUploadEndpoint(endpoint) {
  const normalized=String(endpoint||'').replace(/\/+$/,'');
  return /\/backend-api\/files(?:\/|$)/.test(normalized)||/\/unauth-mweb\/image-uploads(?:\/|$)/.test(normalized);
}
...
async function waitForAttachmentUploadNetwork(tabId,startedAfterMs,timeoutMs=ATTACHMENT_UPLOAD_TIMEOUT_MS) {
  ...
  while(Date.now()<deadline){
    ...
    const processingSeen=uploads.some(item=>/\/backend-api\/files\/process_upload_stream$/.test(String(item.endpoint||''))||/\/unauth-mweb\/image-uploads\/process$/.test(String(item.endpoint||'')));
    const recoverableReuseAbort=uploads.find(isRecoverableAttachmentUploadAbort);
    const processingComplete=[...uploads].reverse().find(item=>isCompletedAttachmentUpload(item,'/backend-api/files/process_upload_stream')||isCompletedAttachmentUpload(item,'/unauth-mweb/image-uploads/process'));
    ...
    const baseCompleted=[...uploads].reverse().find(item=>isCompletedAttachmentUpload(item,'/backend-api/files')||isCompletedAttachmentUpload(item,'/unauth-mweb/image-uploads'));
...
function chatNavigationSupersedesNetworkState(current,nextUrl) {
  ...
  return trackedConversationId!==conversationIdFromUrl(nextUrl);
}

async function resetSupersededChatActivity(tabId,nextUrl) {
  await ensureChatNetworkStateLoaded();
  const current=chatNetworkStateByTab.get(tabId);
  if(!chatNavigationSupersedesNetworkState(current,nextUrl))return false;
...
async function profileInfo() {
  const stored = await chrome.storage.local.get(['profileId','active','connectorInstall','connectorServerFingerprint','headlessWorkerId','headlessWorkerLabel','headlessSourceProfileId','workerEnabled','workerEnabledUpdatedAt',PROFILE_LIFECYCLE_STORAGE_KEY,PROFILE_LIFECYCLE_HISTORY_STORAGE_KEY]);
  profileLifecycleHistory=mergeProfileLifecycleEvents(stored[PROFILE_LIFECYCLE_HISTORY_STORAGE_KEY]||[],profileLifecycleHistory,latestProfileLifecycleEvent||[]);
  ...
  return {
    ...
    lifecycle_event:latestProfileLifecycleEvent||stored[PROFILE_LIFECYCLE_STORAGE_KEY]||null,
    lifecycle_events:profileLifecycleHistory
  };
}

async function headlessExclusiveLockStatus(){
  const stored=await chrome.storage.local.get(['profileId','headlessWorkerId','headlessExclusiveWorkerId','headlessExclusiveLockSetAt']);
  if(stored.headlessWorkerId)return {locked:false,headless:true,worker_id:String(stored.headlessWorkerId||'')};
  const persistedWorkerId=String(stored.headlessExclusiveWorkerId||'').trim();
  const lockSetAt=Math.max(0,Number(stored.headlessExclusiveLockSetAt)||0);
  const pendingStartLock=Boolean(persistedWorkerId&&lockSetAt&&Date.now()-lockSetAt<15000);
  const profileId=String(stored.profileId||'').trim();
  if(!profileId)return persistedWorkerId
    ? {locked:true,headless:false,worker_id:persistedWorkerId,reason:'source_profile_id_missing'}
    : {locked:false,headless:false,worker_id:''};
  try{
    const url=new URL('http://127.0.0.1:9224/headless-lock');
    url.searchParams.set('profile_id',profileId);
    const response=await fetch(url.toString(),{cache:'no-store',signal:AbortSignal.timeout(1800)});
    if(!response.ok)throw new Error(`HTTP ${response.status}`);
    const result=await response.json();
    if(result?.locked===true){
      const workerId=String(result.worker_id||persistedWorkerId||'').trim();
      if(workerId&&workerId!==persistedWorkerId)await chrome.storage.local.set({headlessExclusiveWorkerId:workerId});
      if(lockSetAt)await chrome.storage.local.remove('headlessExclusiveLockSetAt');
      return {locked:true,headless:false,worker_id:workerId};
    }
    if(pendingStartLock)return {locked:true,headless:false,worker_id:persistedWorkerId,reason:'headless_starting'};
    if(persistedWorkerId)await chrome.storage.local.remove(['headlessExclusiveWorkerId','headlessExclusiveLockSetAt']);
    return {locked:false,headless:false,worker_id:''};
  }catch(error){
    if(persistedWorkerId)return {locked:true,headless:false,worker_id:persistedWorkerId,reason:`lock_check_failed:${String(error?.message||error)}`};
    return {locked:false,headless:false,worker_id:'',reason:`lock_check_failed:${String(error?.message||error)}`};
  }
}
...
async function readUnopenedChatResponse(tabs,conversation,args={},expiresAt=0) {
  ...
  // No live stream belongs to this closed conversation. In particular, do not
  // borrow (or reconcile) the root/source tab's network or canonical activity.
...
async function execute(command) {
  ...
  if(action==='set_headless_lock'){
    const tabs=await tabList();
    const busyTabs=tabs.filter(tab=>tab?.busy||tab?.settling||String(tab?.network_state||'')==='generating');
    if(busyTabs.length)throw new Error(`WORKER_BUSY: ${busyTabs.length} ChatGPT tab đang xử lý; chưa khóa ChatGPT để bật headless.`);
    const exclusiveWorkerId=String(args.headless_worker_id||'').trim().slice(0,160);
    if(!exclusiveWorkerId)throw new Error('HEADLESS_LOCK_ID_REQUIRED: Thiếu headless worker id.');
    await chrome.storage.local.set({headlessExclusiveWorkerId:exclusiveWorkerId,headlessExclusiveLockSetAt:Date.now()});
    return {action,ok:true,locked:true,worker_id:exclusiveWorkerId};
  }
  if(action==='clear_headless_lock'){
    const exclusiveWorkerId=String(args.headless_worker_id||'').trim().slice(0,160);
    const stored=await chrome.storage.local.get(['headlessExclusiveWorkerId']);
    const currentWorkerId=String(stored.headlessExclusiveWorkerId||'').trim();
    if(!exclusiveWorkerId||!currentWorkerId||exclusiveWorkerId===currentWorkerId){
      await chrome.storage.local.remove(['headlessExclusiveWorkerId','headlessExclusiveLockSetAt']);
      return {action,ok:true,locked:false,worker_id:currentWorkerId};
    }
    return {action,ok:true,locked:true,worker_id:currentWorkerId,unchanged:true};
  }
  if(action==='close_profile'){
    const tabs=await tabList();
    const busyTabs=tabs.filter(tab=>tab?.busy||tab?.settling||String(tab?.network_state||'')==='generating');
    if(busyTabs.length)throw new Error(`WORKER_BUSY: ${busyTabs.length} ChatGPT tab đang xử lý; không đóng profile khi task chưa hoàn tất.`);
    const exclusiveWorkerId=String(args.headless_worker_id||'').trim().slice(0,160);
    if(exclusiveWorkerId)await chrome.storage.local.set({headlessExclusiveWorkerId:exclusiveWorkerId});
    const windows=(await chrome.windows.getAll({populate:false})).filter(win=>Number.isInteger(win?.id));
    let closedWindowCount=0;
    for(const win of windows){
      windowRemovalIntents.set(win.id,{reason:'browser_control_close_profile',at:Date.now()});
      await recordProfileLifecycleEvent({type:'window_close_requested',reason:'browser_control_close_profile',window_id:win.id}).catch(()=>{});
      try{await chrome.windows.remove(win.id);closedWindowCount+=1;}catch{}
    }
    const remainingWindows=(await chrome.windows.getAll({populate:false}).catch(()=>[])).filter(win=>Number.isInteger(win?.id));
    if(remainingWindows.length)throw new Error(`PROFILE_CLOSE_INCOMPLETE: còn ${remainingWindows.length} cửa sổ Chrome chưa đóng.`);
    return {action,ok:true,closed:true,closed_window_count:closedWindowCount};
  }
  ...
  if(action==='send_chat_request'){
    ...
    const resultForSubmitLifecycle=async(lifecycleEvidence,injectedResult={})=>{
      ...
      pendingConversationByTab.delete(tab.id);
      await clearChatAttachmentOwnership(tab.id,attemptId);
      const evidence=Array.isArray(lifecycleEvidence)?lifecycleEvidence:[];
      ...
      return {
        ...
        network_ack_source:'submit_lifecycle',
        submission_ack_source:'submit_lifecycle',
        submit_lifecycle_acknowledged:true,
        submit_lifecycle_endpoint:String(latestEvidence.endpoint||''),
    ...
    if(!injected?.result?.ok){
      ...
      if(newChat)await removeTabWithReason(tab.id,'send_prepare_failed_new_chat').catch(()=>{});
    ...
    if(injected.result.requires_trusted_submit){
      ...
      try{
        const trustedSubmit=await timedSendPhase('trusted_submit_ms',()=>promiseWithTimeout(
        ...
        if(definitelyNotDispatched&&!attachmentSubmit&&remainingCommandMs()>1500){
          try{
            ...
            await timedSendPhase('fallback_submit_ms',()=>promiseWithTimeout(
...
if(action==='rename_chat'){
  ...
  if(action==='get_chat_response'){
    ...
    if(!tab){
      ...
      if(!conversation)throw new Error('Đoạn chat không còn thuộc 3 chat gần nhất của profile này.');
  ...
  if(action==='close_tab'){await removeTabWithReason(tab.id,'browser_control_close_tab');return {action,target_id:tab.id,ok:true};}
...
async function replaceUnresponsiveChatTab(tab,recoveryUrl,timeoutMs=45000,options) {
  ...
  let replacement=await serializeChatGptTabCreation(async()=>{
    const tabLimit=await chatGptTabLimit();
    let current=(await chrome.tabs.query({url:['https://chatgpt.com/*']})).filter(candidate=>Number.isInteger(candidate.id));
    const preserveOnlyMacTab=tabLimit===MAC_MAX_CHATGPT_TABS&&current.length===1&&current[0].id===replacedTabId;
    if(current.length>=tabLimit&&!preserveOnlyMacTab){
      await releaseChatDebuggerForRecovery(replacedTabId);
      await removeTabWithReason(replacedTabId,'renderer_replacement_precreate');
      replacedTabRemoved=true;
      current=(await chrome.tabs.query({url:['https://chatgpt.com/*']})).filter(candidate=>Number.isInteger(candidate.id));
    }
    if(current.length>=tabLimit&&!preserveOnlyMacTab){
      const summaries=await tabList();
      const recentConversations=await recentConversationList(Math.max(1,tabLimit));
    ...
    if(current.length>=tabLimit&&!preserveOnlyMacTab)throw new Error(`CHAT_TAB_LIMIT_REACHED: Không thể mở tab khôi phục vì profile vẫn có ${current.length} tab ChatGPT được bảo vệ.`);
  ...
  try{
    ...
    await removeTabWithReason(replacement.id,'renderer_replacement_failed').catch(()=>{});
    throw error;
  }
  ...
  if(!replacedTabRemoved){
    await releaseChatDebuggerForRecovery(replacedTabId);
    await removeTabWithReason(replacedTabId,'renderer_replacement_completed').catch(()=>{});
  }
  await persistChatNetworkState();
  recentConversationCache={at:0,items:[]};
  scheduleRealtimeProfilePush(0);
...
chrome.runtime.onInstalled.addListener(()=>{
  ...
  chrome.storage.local.get('codexproReloadTabId').then(async ({codexproReloadTabId})=>{
    if(Number.isInteger(codexproReloadTabId))await removeTabWithReason(codexproReloadTabId,'extension_reload_cleanup').catch(()=>{});
...
chrome.runtime.onMessage.addListener((message,sender,sendResponse) => {
  ...
  if(message?.type==='codexpro-headless-lock-status'){
    headlessExclusiveLockStatus()
      .then(result=>sendResponse({ok:true,...result}))
      .catch(error=>sendResponse({ok:false,locked:true,error:String(error?.message||error)}));
    return true;
  }
