...
function readRunnerMeta(repo) {
  try { return JSON.parse(fs.readFileSync(runnerMetaPath(repo), 'utf8')); } catch { return null; }
}
...
function findRunnerTemplate() {
  for (const candidate of runnerTemplateCandidates()) {
    if (fs.existsSync(path.join(candidate, 'config.cmd')) && fs.existsSync(path.join(candidate, 'run.cmd'))) return candidate;
  }
...
function readConfiguredRunner(root) {
  try {
    ...
    return { root: path.resolve(root), runnerName: runnerNameValue || path.basename(root), repo, githubUrl };
  } catch {
    return null;
  }
}
...
function findLocalConfiguredRunner(repo) {
  ...
  for (const root of configuredRunnerCandidates()) {
    ...
    if (!configured.repo && path.basename(root).toLowerCase() === `actions-runner-${repoName}`) return configured;
  }
  return null;
}
...
function inspectLocalRunnerProcess(root) {
  ...
  const script = [
    `$root = '${escaped}'`,
    "$prefix = $root.TrimEnd('\\') + '\\'",
    "$items = @(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object { ($_.ExecutablePath -and $_.ExecutablePath.StartsWith($prefix, [System.StringComparison]::OrdinalIgnoreCase)) -or ($_.CommandLine -and $_.CommandLine.IndexOf($prefix, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) })",
  ...
  return { running: runningText === 'true', busy: busyText === 'true', listenerPid: Number(pidText) || null };
}
...
function runRunnerConfiguration(root, repo, registrationToken, name) {
  const command = [
    'config.cmd', '--url', commandQuote(`https://github.com/${repo}`), '--token', commandQuote(registrationToken),
    '--name', commandQuote(name), '--work', commandQuote('_work'), '--unattended', '--replace'
  ].join(' ');
...
function launchRunnerProcess(repo, root, name) {
  ...
  runnerProcesses.set(repo, { pid: child.pid, runnerName: name, root, startedAt: new Date().toISOString() });
  child.once('exit', () => {
...
function trackedRunner(repo) {
  ...
  try { process.kill(tracked.pid, 0); return tracked; }
  catch { runnerProcesses.delete(repo); return null; }
}
...
function stopRunnerProcesses(root) {
  ...
  spawnSync('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', script], { windowsHide: true, encoding: 'utf8', timeout: 15000 });
}
...
ipcMain.handle('actions:list', async (_event, input = {}) => {
  ...
  return {
    ...
    runs: (result.data.workflow_runs || []).map((run) => ({
      id: run.id, name: run.name, displayTitle: run.display_title, runNumber: run.run_number, event: run.event,
      branch: run.head_branch, sha: run.head_sha, status: run.status, conclusion: run.conclusion,
      createdAt: run.created_at, updatedAt: run.updated_at, startedAt: run.run_started_at,
      htmlUrl: run.html_url, actor: run.actor?.login || '', attempt: run.run_attempt || 1
    }))
  };
});
...
ipcMain.handle('actions:jobs', async (_event, input = {}) => {
  ...
  return {
    ...
    jobs: (result.data.jobs || []).map((job) => ({
      ...
      runnerName: job.runner_name || '', runnerGroup: job.runner_group_name || '', labels: job.labels || [],
...
ipcMain.handle('runner:status', async (_event, input = {}) => {
  ...
  if (local) {
    ...
    if (token && local.runnerName) {
      try { remote = await findRemoteRunner(repo, token, local.runnerName); }
    ...
    return {
      configured: true, managed, external: !managed, local: true, requiresToken: false,
      running: online || localProcess.running, online, busy, repo, runnerName: local.runnerName,
      root: local.root, pid: localProcess.listenerPid, remoteError
    };
  }
  ...
  if (meta) {
    ...
    let remoteError = '';
    if (token) {
      try { remote = await findRemoteRunner(repo, token, meta.runnerName); }
      catch (error) { remoteError = error.message || String(error); }
    }
    ...
    return {
      configured: true, managed: true, external: false, local: true, requiresToken: false,
      running: online || processState.running || Boolean(tracked), online,
      busy: remote ? Boolean(remote.busy) : processState.busy, repo, runnerName: meta.runnerName,
      root: meta.root, pid: processState.listenerPid || tracked?.pid || null, remoteError
    };
  }

  if (!token) {
    return {
      configured: false, managed: false, external: false, local: false, requiresToken: true,
      running: false, online: false, busy: false, repo
    };
  }
  ...
  try {
    ...
    return {
      configured: runners.length > 0, managed: false, external: runners.length > 0, local: false,
      requiresToken: false, running: onlineRunners.length > 0, online: onlineRunners.length > 0,
      busy: onlineRunners.some((runner) => Boolean(runner.busy)), repo, runnerName: active?.name || '',
      runnerCount: runners.length, remoteError: ''
    };
  } catch (error) {
    return {
      configured: false, managed: false, external: false, local: false, requiresToken: false,
      running: false, online: false, busy: false, repo, remoteError: error.message || String(error)
    };
  }
});
...
ipcMain.handle('runner:start', async (_event, input = {}) => {
  ...
  if (local) {
    ...
    if (processState.running) {
      return { ok: true, alreadyRunning: true, repo, runnerName: local.runnerName, online: true, busy: processState.busy, managed: false, external: true, local: true };
    }
    const pid = launchRunnerProcess(repo, local.root, local.runnerName);
    return { ok: true, alreadyRunning: false, repo, runnerName: local.runnerName, pid, root: local.root, online: true, busy: false, managed: false, external: true, local: true };
  }
  ...
  if (currentMeta?.runnerName) {
    try {
      ...
      if (currentRemote?.status === 'online') {
        return { ok: true, alreadyRunning: true, repo, runnerName: currentMeta.runnerName, online: true, busy: Boolean(currentRemote.busy), managed: true, external: false, local: true };
      }
    } catch {}
  } else {
    try {
      ...
      if (online) {
        return { ok: true, alreadyRunning: true, repo, runnerName: online.name, online: true, busy: Boolean(online.busy), managed: false, external: true, local: false };
      }
    } catch {}
  }
  ...
  writeRunnerMeta(repo, { repo, runnerName: name, root: prepared.root, template: prepared.source, configuredAt: new Date().toISOString() });
  const pid = launchRunnerProcess(repo, prepared.root, name);
  return { ok: true, alreadyRunning: false, repo, runnerName: name, pid, root: prepared.root, online: true, busy: false, managed: true, external: false, local: true };
});
