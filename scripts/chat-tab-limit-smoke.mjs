...
assert.match(worker, /recover_chat_tab[\s\S]*?createChatGptTab\(\{url:'https:\/\/chatgpt\.com\/',active:focusWindow\},'recover_chat_tab_new'\)/, "fresh-chat recovery must use the capped tab creator and respect explicit focus intent");
assert.match(worker, /newChat[\s\S]*?createChatGptTab\(\{url:'https:\/\/chatgpt\.com\/',active:false\},'send_chat_request_new'\)/, "new chat continuation requests must use the capped tab creator without stealing foreground focus");
assert.match(worker, /tab=await createChatGptTab\(\{url:'https:\/\/chatgpt\.com\/c\/'\+conversationId,active:false\},'send_chat_request_existing'\)/, "conversation reopens for send must use the capped tab creator in the background");
...
assert.doesNotMatch(worker, /function focusNewChatGptTab|forceChatFocus/, "central ChatGPT tab creation must preserve the caller's background/foreground intent");
...
assert.match(managerMain, /async function recoverProfileChatTab[\s\S]*?focus_window: !silent[\s\S]*?silent \? \{ ok: false, skipped: true, source: "auto-recovery" \}/, "silent recovery must stay in the background while manual recovery may focus Chrome");
assert.match(server, /focus_window: z\.boolean\(\)\.optional\(\)[\s\S]*?explicit recover_chat_tab/, "MCP input validation must preserve the explicit recovery focus flag");
assert.doesNotMatch(managerMain, /NEW_CHAT_FOCUS_FAILED|let newChatWindowFocus = null/, "new-chat sends must not force Chrome into the foreground");
assert.doesNotMatch(managerMain, /owner\.minimize\(\)/, "recovery must never hide the Manager window");
assert.match(managerMain, /const restoreManagerFocus = Boolean\([\s\S]*?owner\.isFocused\(\)[\s\S]*?if \(owner\.isMinimized\(\)\) owner\.restore\(\);[\s\S]*?owner\.focus\(\);/, "send fallback must restore the Manager only when it owned focus before dispatch");
