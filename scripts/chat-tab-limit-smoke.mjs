...
assert.match(worker, /newChat[\s\S]*?createChatGptTab\(\{url:'https:\/\/chatgpt\.com\/',active:true\},'send_chat_request_new'\)/, "new chat continuation requests must foreground-focus the capped tab creator");
assert.match(worker, /tab=await createChatGptTab\(\{url:'https:\/\/chatgpt\.com\/c\/'\+conversationId,active:true\},'send_chat_request_existing'\)/, "conversation reopens for send must foreground-focus the reopened tab");
...
assert.match(worker, /createArgs\?\.active===true[\s\S]*?chrome\.tabs\.update\(created\.id,\{active:true\}\)[\s\S]*?chrome\.windows\.update\(created\.windowId,\{focused:true\}\)/, "foreground ChatGPT tab creation must activate the tab and focus its Chrome window");
