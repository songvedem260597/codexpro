...
assert.match(worker, /const heartbeat=setInterval\(\(\)=>\{void chrome\.storage\.local\.get\('workerEnabled'\)[\s\S]*?fetch\(`\$\{BRIDGE\}\/register`/, "long extension commands must keep enabled profiles alive without resurrecting a disabled worker");
