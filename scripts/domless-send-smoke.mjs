...
import './message-stream-language-smoke.mjs';
...
const workerExtensionCurrent = Function("WORKER_EXTENSION_VERSION", "WORKER_EXTENSION_BUILD_ID", `${workerExtensionCurrentSource}; return workerExtensionCurrent;`)("0.5.135", "send-button-ack-v13");
assert.equal(workerExtensionCurrent({ extension_version: "0.5.127", extension_build_id: "old-build" }), false, "older version with old build must remain stale");
assert.equal(workerExtensionCurrent({ extension_version: "0.5.135" }), false, "same-version runtime with missing build id must be stale");
assert.equal(workerExtensionCurrent({ extension_version: "0.5.135", extension_build_id: "stale-same-version" }), false, "same-version stale runtime must be rejected when build id differs");
assert.equal(workerExtensionCurrent({ extension_version: "0.5.135", extension_build_id: "send-button-ack-v13" }), true, "exact version and build id must be accepted without a reload");
assert.equal(workerExtensionCurrent({ extension_version: "0.5.135", extension_build_id: "future-but-unexpected" }), false, "newer version must not be considered current without the expected build identity");
assert.equal(workerExtensionCurrent({ extension_version: "0.5.134", extension_build_id: "send-button-ack-v13" }), false, "wrong extension version must remain stale even when build id matches");
...
assert.doesNotMatch(sendBlock, /prepareError[\s\S]{0,800}waitForNetworkGeneration/, "traffic that started before trusted click must not turn a prepare failure into a submitted result");
assert.ok(cleanup > timeoutCatch, "a recoverable prepare failure may clean only its owned, unsubmitted draft");
assert.ok(sendButtonPrimary >= 0, "the verified visible Send button must be the primary submit path");
assert.ok(postClickVerification > sendButtonPrimary, "the primary button click must enter a short strong-signal verification window");
assert.ok(retryClick > postClickVerification, "an unchanged exact owned draft may receive one additional button click");
assert.ok(postRetryVerification > retryClick, "the one safe retry must be followed by bounded verification");
assert.doesNotMatch(sendBlock, /trustedSubmitChatComposerTab\(tab\.id,attemptId,text\)/, "trusted Enter must not remain the preferred send mechanism");
...
assert.doesNotMatch(sendBlock, /waitForNetworkGeneration\([\s\S]*?NETWORK_START_TIMEOUT_MS/, "healthy post-click verification must not contain the old 30 second ACK wait");
assert.match(sendBlock, /send_phase_timings:[\s\S]*?extension_total_ms/, "send diagnostics must expose total extension timing");
assert.match(sendBlock, /timedSendPhase\('trusted_click_ms'/, "send diagnostics must time trusted Send-button dispatch");
assert.match(sendBlock, /timedSendPhase\('post_click_verification_ms'/, "send diagnostics must time bounded post-click verification");
...
assert.match(sendBlock, /submit_path:'trusted-send-button'/);
assert.match(sendBlock, /'trusted-send-button-retry'/);
assert.match(sendBlock, /retry_click_count:1/);
...
assert.match(sendBlock, /activateChatTabForTrustedSend\(tab\.id,tabActivationOrigin\)/, "trusted button input must temporarily activate and focus its target tab");
assert.match(sendBlock, /restoreChatTabAfterTrustedSend\(tabActivation\)/, "temporary target activation must be restored after the send decision");
assert.match(sendBlock, /'SEND_UNCERTAIN'/, "ambiguous composer departure must remain explicitly uncertain");
assert.match(sendBlock, /composer_matches_payload_after_click/, "post-click diagnostics must preserve exact composer payload evidence");
assert.match(sendBlock, /matching_user_message_after_click/, "post-click diagnostics must preserve exact transcript evidence");
assert.match(sendBlock, /generation_endpoint/, "post-click diagnostics must preserve the generation endpoint");
...
assert.match(sendBlock, /classifyChatPrepareRecovery\(prepareError,prepareAttempt,oneShotRecovery\)[\s\S]*?if\(!hardRendererHang\)await cleanupAttempt\(\)[\s\S]*?prepareRecovery\.mode==='replace-tab'[\s\S]*?replaceUnresponsiveChatTab\([\s\S]*?continue;/, "a hard renderer prepare timeout must skip DOM cleanup, replace the stuck tab and retry exactly once before trusted submit");
assert.doesNotMatch(sendBlock, /submitted_by:'prepare-renderer-timeout'|PREPARE_UNCERTAIN:/, "hard prepare timeout must not return early before the bounded replace-tab recovery path");
assert.match(sendBlock, /const limit=hardRendererHang\?\{reached:false\}:await probeConversationLimit\(tab\.id\)[\s\S]*?submitted_by:'prepare-renderer-recovery-failed'[\s\S]*?send_uncertain:false[\s\S]*?PREPARE_RECOVERY_FAILED:/, "failed hard-renderer replacement must stay definitely unsent and must not queue another DOM probe into the hung tab");
...
assert.equal(trustedSubmitRendererRoundTrips, 1, "the retained legacy Enter helper must remain bounded if another command still calls it");
assert.match(trustedSubmitSourceForLatency, /startCdpChatNetworkTracker\(tabId\)[\s\S]*?Emulation\.setFocusEmulationEnabled[\s\S]*?func:focusChatComposerForSubmitPage[\s\S]*?Input\.dispatchKeyEvent/, "the isolated legacy Enter helper must keep its pre-dispatch safety ordering");
assert.doesNotMatch(trustedSubmitSourceForLatency, /settleBlockingModal|dismissKnownBlockingChatModalPage|const \[refocused\]|const \[finalFocus\]/, "the legacy helper must not fan out into redundant renderer verification round trips");
...
assert.match(sendBlock, /waitForAttachmentUploadNetwork\(tab\.id,submitStartedAt,/, "attachment upload evidence must use the pre-click preparation cutoff separately from generation ACK evidence");
assert.doesNotMatch(sendBlock, /attachmentSubmit\?/, "text and attachment sends must use the same verified visible-button primary path after preparation");
assert.match(sendBlock, /trustedSubmitChatSendButtonTab\(tab\.id,attemptId,text,/, "attachment sends must use the verified trusted Send button after upload ACK");
...
assert.match(sendBlock, /generationAckStartedAfterMs=Number\(trustedSubmit\.click_dispatch_started_at\)\|\|Date\.now\(\)/, "follow-up ACK detection must use the exact fresh click cutoff so stale network evidence cannot satisfy the new send");
...
console.log("✓ ChatGPT verified Send-button primary smoke test passed");
