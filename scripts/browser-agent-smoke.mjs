...
assert.match(worker, /const WORKER_RUNTIME_BUILD_ID = 'send-post-ack-scope-v1'/, "extension heartbeat must publish the hotfix runtime build identity");
...
assert.match(managerMain, /const WORKER_EXTENSION_BUILD_ID = "send-post-ack-scope-v1";/, "Manager backend must require the exact extension runtime build id");
...
assert.match(managerUi, /const WORKER_EXTENSION_VERSION = "0\.5\.126";[\s\S]*?const WORKER_EXTENSION_BUILD_ID = "send-post-ack-scope-v1";[\s\S]*?function extensionReady\(profile, targetVersion = WORKER_EXTENSION_VERSION\)/, "Manager UI must classify same-version build-id mismatches with an exact runtime identity helper");
