...
assert.match(worker, /newChat[\s\S]*?createChatGptTab\(\{url:'https:\/\/chatgpt\.com\/',active:true\},'send_chat_request_new'\)/, "automatic rollover must create the replacement ChatGPT tab focused in the foreground");
...
assert.match(worker, /const forceChatFocus=isChatGptTabUrl\(requestedUrl\)[\s\S]*?effectiveArgs=forceChatFocus\?\{\.\.\.createArgs,active:true\}:createArgs[\s\S]*?focusNewChatGptTab\(tab,source\)/, "audited tab creation must force foreground focus for all ChatGPT opens, including recovery and continuation paths");
