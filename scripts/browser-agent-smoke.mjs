...
assert.match(managerMain, /confirmationDeadline[\s\S]*?workerExtensionCurrent\(profile\)/, "worker update must wait for a heartbeat confirming both extension version and runtime build id");
assert.match(managerUi, /const WORKER_EXTENSION_VERSION = "0\.5\.126";[\s\S]*?const WORKER_EXTENSION_BUILD_ID = "send-post-ack-scope-v1";[\s\S]*?extensionReady\(profile\)/, "Manager UI must classify same-version build-id mismatches as needing update");
assert.match(managerUi, /const hasSafeOutdatedWorker = profiles\.some\(\(profile\) => \{[\s\S]*?if \(!profile\?\.connected \|\| extensionReady\(profile\)\) return false;/, "Manager auto-update must use exact runtime identity instead of version-only gating");
