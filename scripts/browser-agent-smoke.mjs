...
assert.match(workerSendSource, /domActivity\.connection_interrupted\|\|domActivity\.message_delivery_timed_out[\s\S]*?probeCanonicalActivity\(tab\.id,targetConversationId,true\)[\s\S]*?canonical\?\.ok&&canonical\.response_ready&&!canonical\.busy[\s\S]*?send_preflight_canonical/, "send must clear a stale interrupted DOM busy guard when canonical proves the previous turn completed");
