...
assert.match(managerUi, /const nextTotalMessageCount = protectAuthoritativeSnapshot[\s\S]*?authoritativeSnapshot\.totalMessageCount[\s\S]*?result\.total_message_count[\s\S]*?totalMessageCount: nextTotalMessageCount/, "Manager response state must retain the authoritative complete message count while feeding stalled-rollover recovery state");
...
assert.match(server, /const responseFinished[\s\S]*?!responseHasStrongerNetworkStreamEvidence\(result\)[\s\S]*?const terminalOutcome[\s\S]*?\? "cancelled"[\s\S]*?\? "failed"[\s\S]*?: null;[\s\S]*?worker_job_completion_pending_finalize = true/, "durable browser jobs must keep a safe completed response pending explicit finalization, and stronger network evidence must keep even that pending state provisional");
...
assert.match(managerMain, /async function recoverProfileChatTab[\s\S]*?action: "recover_chat_tab"[\s\S]*?focusChromeWindow\(newChat \? "ChatGPT" : \(title \|\| "ChatGPT"\)\)/, "hung-tab recovery must native-focus Chrome even for automatic/silent recovery paths");
assert.doesNotMatch(managerMain, /silent \? \{ ok: false, skipped: true, source: "auto-recovery" \}/, "automatic hang recovery must no longer skip Chrome foreground focus");
assert.match(managerMain, /let newChatWindowFocus = null[\s\S]*?if \(newChat && [\s\S]*?const newTabActivation = await localMcpToolInSession[\s\S]*?action: "activate_tab"[\s\S]*?focusChromeWindow\(String\(newTabActivation\?\.title \|\| "ChatGPT"\)\)[\s\S]*?NEW_CHAT_FOCUS_FAILED/, "new-chat continuation sends must activate the created tab and verify native Windows foreground focus using the tab's current title before returning");
...
assert.match(worker, /async function focusNewChatGptTab[\s\S]*?chrome\.tabs\.update\(tab\.id,\{active:true\}\)[\s\S]*?chrome\.windows\.update\(windowId,\{state:'maximized'\}\)[\s\S]*?chrome\.windows\.update\(windowId,\{focused:true\}\)/, "every newly created ChatGPT tab must be activated and its Chrome window focused/maximized");
assert.match(worker, /async function createChatGptTab\(createArgs=\{\},source='chat_tab_create'\)[\s\S]*?chrome\.tabs\.update\(reusable\.id,\{url,active:true\}\)[\s\S]*?focusNewChatGptTab\(updated,source\)[\s\S]*?chrome\.tabs\.create\(\{\.\.\.createArgs,active:true\}\)[\s\S]*?focusNewChatGptTab\(created,source\)/, "Mac-aware ChatGPT tab creation must foreground both the reused sole macOS tab and newly created ChatGPT tabs");
assert.match(worker, /createChatGptTab\(\{url:'https:\/\/chatgpt\.com\/',active:true\},'send_chat_request_new'\)/, "new-chat continuation sends must open the new ChatGPT tab active");
...
assert.match(worker, /const MAX_CHATGPT_TABS = 2[\s\S]*?const MAC_MAX_CHATGPT_TABS = 1/, "worker must cap non-mac profiles at two ChatGPT tabs while preserving macOS at one");
...
assert.match(worker, /current\.active&&!allowActiveIdle[\s\S]*?current\.pinned\|\|current\.audible[\s\S]*?pendingConversationBlocksChatTabCleanup\(tabId\)[\s\S]*?chatAttachmentOwnershipByTab\.has\(tabId\)[\s\S]*?browserMutationTailsByTab\.has\(tabId\)[\s\S]*?debuggerBusy[\s\S]*?networkState\.busy\|\|canonicalActivity\.busy\|\|domActivity\?\.busy/, "worker must preserve the macOS active-tab exception while re-checking fresh pending, attachment, automation, debugger, network, canonical, and DOM guards before cleanup");
