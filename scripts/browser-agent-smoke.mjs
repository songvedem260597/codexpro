...
assert.equal(CHATGPT_CONVERSATION_MESSAGE_LIMIT, 5, "ChatGPT conversations must roll over after five completed tasks, not five raw transcript messages");
assert.equal(CHATGPT_CONVERSATION_FAST_MESSAGE_LIMIT, 10, "fast active conversations must be allowed ten completed tasks");
assert.equal(CHATGPT_CONVERSATION_FAST_ACTIVITY_MS, 10 * 60 * 1000, "fast activity qualification must use a ten-minute window");
...
const fastConversation = { messageCount: 5, totalMessageCount: 10, activityStartedAt: new Date(fastConversationStartedAt).toISOString(), messages: [{ role: "assistant", text: "task five complete" }] };
assert.equal(shouldQualifyFastMessageLimit({ ...fastConversation, messageCount: 4 }, fastConversationStartedAt + 60_000), false, "a conversation must complete five tasks before qualifying for the fast limit");
assert.equal(shouldQualifyFastMessageLimit(fastConversation, fastConversationStartedAt + CHATGPT_CONVERSATION_FAST_ACTIVITY_MS - 1), true, "five completed tasks inside ten minutes must qualify for the fast limit");
assert.equal(conversationMessageLimit(fastConversation, fastConversationStartedAt + CHATGPT_CONVERSATION_FAST_ACTIVITY_MS - 1), 10, "a qualifying fast conversation must remain on the current tab at task five");
assert.equal(shouldRolloverConversation(fastConversation, undefined, fastConversationStartedAt + CHATGPT_CONVERSATION_FAST_ACTIVITY_MS - 1), false, "the next task must not roll over after five fast completed tasks");
assert.equal(conversationMessageLimit({ ...fastConversation, fastMessageLimitQualified: true }, fastConversationStartedAt + CHATGPT_CONVERSATION_FAST_ACTIVITY_MS * 2), 10, "fast-limit qualification must remain sticky after the initial window");
assert.equal(conversationMessageLimit(fastConversation, fastConversationStartedAt + CHATGPT_CONVERSATION_FAST_ACTIVITY_MS), 5, "a conversation reaching task five at ten minutes must retain the normal limit");
assert.equal(shouldRolloverConversation(fastConversation, undefined, fastConversationStartedAt + CHATGPT_CONVERSATION_FAST_ACTIVITY_MS), true, "a normal-speed conversation must roll over after five completed tasks");
assert.equal(shouldRolloverConversation({ ...fastConversation, messageCount: 10, fastMessageLimitQualified: true }, 10), true, "a qualified fast conversation must roll over after ten completed tasks");
assert.equal(shouldRolloverConversation({ messageCount: 5, totalMessageCount: 11, busy: true, messages: [{ role: "assistant", text: "task five complete" }, { role: "user", text: "change one more thing" }] }), false, "a busy task at the limit must keep accepting follow-ups in the current conversation");
assert.equal(shouldRolloverConversation({ messageCount: 5, totalMessageCount: 11, messages: [{ role: "user", text: "follow-up still awaiting assistant" }] }), false, "an open user follow-up at the limit must not be forced into a new tab");
...
assert.equal(longRunningChatWatchdogCandidate({ ...longTaskProfile, conversation_tabs: [{ ...longTaskProfile.conversation_tabs[0], message_stream_error: true }] }, longTaskJobs, Date.parse("2026-09-01T01:00:00.000Z")), null, "message-stream failures must bypass the reload watchdog and go straight to continuation recovery");
...
const [browserOps, worker, tabPolicyWorker, networkPolicyWorker, responsePolicyWorker, browserControlWorker, server, repoTaskToolsSource, workerJobToolsSource, httpSource, bridge, profileState, managerMain, managerChatCache, managerChatDiagnostics, managerPreload, managerUi, managerWorkerUpdateDialog, managerChatComposer, managerStyles, managerDiagnosticView, managerAppDropdown, managerChatScroll, manifestText, connectorInstaller, popupHtml, popupJs] = await Promise.all([
  ...
  Promise.all([
    readFile(join(root, "manager", "src", "hooks", "use-profile-actions.js"), "utf8"),
    readFile(join(root, "manager", "src", "hooks", "use-project-actions.js"), "utf8"),
    readFile(join(root, "manager", "src", "hooks", "use-chat-send-actions.js"), "utf8"),
    readFile(join(root, "manager", "src", "hooks", "use-chat-response-loader.js"), "utf8"),
    readFile(join(root, "manager", "src", "hooks", "use-chat-response-cache.js"), "utf8"),
    readFile(join(root, "manager", "src", "hooks", "use-chat-recovery.js"), "utf8"),
    readFile(join(root, "manager", "src", "hooks", "use-chat-session.js"), "utf8"),
    readFile(join(root, "manager", "src", "hooks", "use-runtime-status.js"), "utf8"),
    readFile(join(root, "manager", "src", "hooks", "use-chat-viewport.js"), "utf8"),
    readFile(join(root, "manager", "src", "features", "chat", "chat-activity.js"), "utf8"),
    readFile(join(root, "manager", "src", "features", "chat", "chat-conversation-utils.js"), "utf8"),
    readFile(join(root, "manager", "src", "features", "chat", "chat-ui-actions.js"), "utf8"),
    readFile(join(root, "manager", "src", "features", "profiles", "browser-profiles-section.jsx"), "utf8"),
    readFile(join(root, "manager", "src", "features", "chat", "chat-modal.jsx"), "utf8"),
...
assert.match(managerUi, /const profileTaskSummary = profileTaskSummaryState\(\{ profile, cachedTitle: (?:profileTaskLabels|taskLabels)\[profile\.profile_id\], working, settling \}\);[\s\S]*?const profileTaskLabel = profileTaskSummary\.title;[\s\S]*?<span>\{profileTaskSummary\.label\}<\/span>[\s\S]*?<strong>\{profileTaskLabel\}<\/strong>/, "Manager must retain the last AI task title after completion through the shared profile task summary state");
...
assert.match(managerStyles, /\.diagnostic-toolbar \{[^}]*grid-template-columns: minmax\(240px, 1fr\) auto[\s\S]*?\.diagnostic-filter-row \{[^}]*repeat\(5, minmax\(0, 1fr\)\)/, "Diagnostic toolbar must reserve a full row for five unclipped filters");
assert.match(managerMain, /createDiagnosticIpcRegistrar[\s\S]*?function handle\(channel, options, handler\)[\s\S]*?envelopeError[\s\S]*?durationMs >= Number\(options\.slowMs\)[\s\S]*?catch \(error\)/, "Manager IPC diagnostics must capture envelope failures, slow operations, and thrown errors centrally through the extracted registrar");
...
const managerReadyIndex = managerMain.indexOf('app.whenReady().then');
const managerCreateWindowIndex = managerMain.indexOf('createWindow();', managerReadyIndex);
const managerFreshnessIndex = managerMain.indexOf('void ensureFreshRuntimeAfterManagerStart();', managerCreateWindowIndex);
assert.ok(managerReadyIndex >= 0 && managerCreateWindowIndex > managerReadyIndex && managerFreshnessIndex > managerCreateWindowIndex, "Manager startup must run the runtime freshness check after opening the UI, allowing non-blocking setup work in between");
...
assert.match(managerUi, /className="button primary profile-chat[^"]*"[\s\S]*?disabled=\{!profile\.connected \|\| !connectorInstalled\}[\s\S]*?CodexPro sẽ tự mở tab ChatGPT khi gửi/, "an online prepared profile must allow opening the composer before a ChatGPT tab exists even when the button has additional visual classes");
...
assert.match(managerUi, /profile\.chatgpt_tab_count[\s\S]*?<span>\{chatGptTabCount\} tab<\/span>/, "profile metadata must derive the shortened tab label from ChatGPT tab count instead of every Chrome tab");
...
assert.match(managerUi, /(?:async function stopControlTask|const stopControlTask = useCallback\(async)[\s\S]*?api\.stopProfileTask[\s\S]*?onStop=\{\(task\) => void stopControlTask\(task\)\}/, "Control Center must wire task stop from UI to preload");
...
assert.match(managerUi, /const cacheEntry = \{[\s\S]*?totalMessageCount: Number\(response\?\.totalMessageCount\)[\s\S]*?responseCacheSaveQueue\.current\.enqueue\(key, cacheEntry/, "the complete conversation count must survive Manager cache hydration");
assert.match(managerMain, /const managerChatCache = createManagerChatCache\(\{[\s\S]*?home: codexProHome,[\s\S]*?onMetrics:/, "Manager must wire the extracted chat cache service into the main process");
...
assert.match(managerUi, /tab\?\.conversation_limit_reached[\s\S]*?recoveryContinuationSnapshot\(profile, conversationId, targetTab\)[\s\S]*?conversation-limit-auto-rollover-start[\s\S]*?rolloverFullConversation\(profile, conversationId,[\s\S]*?continuation_reason: "limit"[\s\S]*?conversation-limit-auto-rollover-done/, "Manager must proactively preserve context and create a continuation chat as soon as a selected/active tab reports the terminal length banner");
assert.match(worker, /newChat[\s\S]*?createChatGptTab\(\{url:'https:\/\/chatgpt\.com\/',active:true\},visualWatchdog\?'send_chat_request_watchdog':'send_chat_request_new',\{visualWatchdog\}\)/, "automatic rollover must create the replacement ChatGPT task tab through the foreground new-chat path without consuming the Watchdog role");
...
assert.match(managerUi, /onClick=\{\(\) => rendererUnresponsive \? onRecoverProfile\(profile\) : onOpenProfile\(profile\)\}/, "the profile card must route an unresponsive conversation through recovery and otherwise delegate to the current open-profile action");
...
assert.match(managerUi, /cachedResponseIsFresh = useCallback\(\(profile, conversationId, cached\) => \{[\s\S]*?cached\?\.responseReady !== true/, "an unverified cached assistant fragment must never be accepted as the final ChatGPT response");
...
assert.match(worker, /function networkStreamCompletedForCurrentGeneration\([\s\S]*?networkState\?\.network_state==='completed'[\s\S]*?networkStream\.in_progress!==true[\s\S]*?networkStreamMatchesCurrentGeneration\(networkState,networkStream\)[\s\S]*?networkStreamCompletedCurrent=networkStreamCompletedForCurrentGeneration/, "worker must retain a terminal current-generation stream even when the capture bridge does not emit an explicit completed flag");
...
assert.match(managerMain, /const previousTaskId = String\(payload\?\.previousTaskId[\s\S]*?let taskId = previousTaskId \|\| `cpt_/, "chat continuation and active-task adjustments must preserve the original prepared task id");
...
assert.match(managerUi, /repoTaskDispatchedAt: String\(result\?\.repo_task_dispatched_at/, "initial sends must retain their dispatch boundary for task verification");
const repoTaskVerificationSource = managerUi.slice(managerUi.indexOf("async function verifyRepoTaskUse"), managerUi.indexOf("\n  useEffect(() => {", managerUi.indexOf("async function verifyRepoTaskUse")));
assert.doesNotMatch(repoTaskVerificationSource, /api\.sendProfileRequest\(/, "repo task verification must never resend the user's message");
assert.match(repoTaskVerificationSource, /repoTaskStatus: "activation-failed"/, "task activation failure must remain distinct from a submitted message");
...
assert.match(worker, /RENDERER_SEND_PREFLIGHT_TIMEOUT_MS[\s\S]*?function probeChatRendererForSend[\s\S]*?Runtime\.evaluate[\s\S]*?function ensureChatRendererReadyForSend[\s\S]*?RENDERER_NEEDS_FOREGROUND/, "worker send must use a pure CDP renderer preflight and request foreground recovery before DOM prepare when the profile is frozen");
assert.match(managerMain, /function localMcpChatSendWithRendererRecovery[\s\S]*?rendererNeedsForegroundError[\s\S]*?activate_tab[\s\S]*?focusChromeWindow\(title\)[\s\S]*?renderer_native_wake_retry/, "Manager must reproduce the user's manual profile-open recovery automatically before retrying a definitely-unsent request");
assert.match(managerMain, /const result = await localMcpChatSendWithRendererRecovery\(session, profile,[\s\S]*?action: "send_chat_request"/, "profile sends must go through renderer foreground recovery instead of calling the bridge send directly");
assert.match(managerMain, /async function recoverProfileChatTab[\s\S]*?focus_window: true[\s\S]*?const windowFocus = await focusChromeWindow/, "automatic and explicit hang recovery must foreground-focus Chrome");
assert.match(managerMain, /let keepChromeFocused = payload\?\.newChat === true[\s\S]*?chatgpt_tab_auto_opened[\s\S]*?if \(!keepChromeFocused && restoreManagerFocus/, "new-chat continuation sends must keep native Windows foreground focus on Chrome");
assert.doesNotMatch(managerMain, /owner\.minimize\(\)/, "recovery must never minimize the Manager window");
assert.match(managerMain, /if \(!keepChromeFocused && restoreManagerFocus[\s\S]*?if \(owner\.isMinimized\(\)\) owner\.restore\(\);[\s\S]*?owner\.focus\(\);/, "renderer wake fallback may return focus to Manager only when Chrome continuation focus is not required");
...
assert.match(worker, /const MAX_CHATGPT_TABS = 3/, "worker must cap ChatGPT tabs per Chrome profile at three so two task tabs can coexist with the protected Watchdog tab");
assert.match(worker, /const MAX_TASK_CHATGPT_TABS = 2/, "worker must reserve the third ChatGPT tab slot instead of allowing three normal task tabs");
assert.match(worker, /VISUAL_WATCHDOG_STORAGE_KEY = 'codexproVisualWatchdogTabV1'/, "worker must persist the protected Visual Watchdog tab identity");
assert.match(worker, /TASK_TAB_LIMIT_REACHED:[\s\S]*?slot thứ 3 được dành riêng cho Watchdog/, "normal task creation must not consume the dedicated Watchdog slot");
assert.match(worker, /const visualWatchdog=Boolean\(args\.visual_watchdog\)/, "Watchdog sends must be marked before tab selection and creation");
assert.match(worker, /isRegisteredWatchdog\(current\)\|\|current\.active/, "tab cleanup must never close the registered Visual Watchdog tab");
...
assert.match(worker, /current\.active\|\|current\.pinned\|\|current\.audible[\s\S]*?pendingConversationBlocksChatTabCleanup\(tabId\)[\s\S]*?chatAttachmentOwnershipByTab\.has\(tabId\)[\s\S]*?browserMutationTailsByTab\.has\(tabId\)[\s\S]*?debuggerBusy[\s\S]*?networkState\.busy\|\|canonicalActivity\.busy\|\|domActivity\?\.busy/, "worker must re-check fresh work, attachment, automation, transient debugger, network, canonical, and DOM protection signals before closing a tab");
...
assert.match(bridge, /profile\.runtimeBuildId = String\(source\.runtime_build_id \?\? ""\)\.trim\(\)\.slice\(0, 120\)/, "a live extension heartbeat must clear persisted runtime identity when runtime_build_id is missing");
assert.doesNotMatch(bridge, /source\.runtime_build_id \?\? profile\.runtimeBuildId/, "a live extension heartbeat must never inherit a persisted runtime build id");
