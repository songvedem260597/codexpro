...
assert.match(managerMain, /function expectedRuntimeBuildId[\s\S]*?dist[\s\S]*?http\.js[\s\S]*?function ensureFreshRuntimeAfterManagerStart[\s\S]*?runtimeBuildId[\s\S]*?controlServer\("restart"\)[\s\S]*?runtime-build-refreshed/, "Manager startup must restart a stale loaded runtime and verify the replacement build");
