...
assert.equal(manifest.version, "0.5.109", "busy follow-up steering must be packaged in a reloadable extension version");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.109";/, "Manager updater must require the worker version with serialized busy follow-up steering");
