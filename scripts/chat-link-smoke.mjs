...
assert.equal(manifest.version, "0.5.117", "foreground-focused ChatGPT tab creation and prior browser fixes must be packaged in a reloadable extension version");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.117";/, "Manager updater must require the worker version that foreground-focuses newly opened ChatGPT tabs");
