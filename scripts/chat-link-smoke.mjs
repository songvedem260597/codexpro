...
assert.equal(manifest.version, "0.5.118", "foreground-focused ChatGPT tab creation and proactive full-conversation rollover must be packaged in a reloadable extension version");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.118";/, "Manager updater must require the worker version that foreground-focuses rollover tabs and detects terminal conversation length");
