...
assert.equal(manifest.version, "0.5.120", "message-stream rollover recovery must be packaged in a reloadable extension version");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.120";/, "Manager updater must require the worker version that detects message-stream errors");
