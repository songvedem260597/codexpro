...
assert.equal(manifest.version, "0.5.81", "worker link support must remain packaged with the latest response-reader fix");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.81";/, "Manager updater must require the worker version that contains link extraction and complete response reads");
