...
assert.equal(manifest.version, "0.5.85", "worker generation validation must remain packaged with the latest response-reader fix");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.85";/, "Manager updater must require the worker version that rejects stale and incomplete responses");
