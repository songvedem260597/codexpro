...
assert.equal(manifest.version, "0.5.116", "two-tab profile cap and prior browser fixes must be packaged in a reloadable extension version");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.116";/, "Manager updater must require the worker version with the two-tab profile cap");
