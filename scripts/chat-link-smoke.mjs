...
assert.equal(manifest.version, "0.5.105", "the macOS chat-read and tab lifecycle fixes must be packaged in a reloadable extension version");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.105";/, "Manager updater must require the worker version containing the macOS chat-read and tab lifecycle fixes");
