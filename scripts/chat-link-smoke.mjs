...
assert.equal(manifest.version, "0.5.91", "worker popup and enable toggle must remain packaged with the latest recovery fixes");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.91";/, "Manager updater must require the worker version with the stuck-tab recovery fix");
