...
assert.equal(manifest.version, "0.5.82", "worker link support must remain packaged with the latest response-reader fix");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.82";/, "Manager updater must require the worker version that contains link extraction and complete response reads");
