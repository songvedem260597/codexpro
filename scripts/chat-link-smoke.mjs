...
assert.equal(manifest.version, "0.5.88", "worker timing telemetry must remain packaged with the latest response-reader fixes");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.88";/, "Manager updater must require the worker version with response timing telemetry");
