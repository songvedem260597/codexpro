...
assert.equal(manifest.version, "0.5.113", "completed network-stream, renderer wake recovery, and direct plus console/log 429 diagnostics must be packaged in a reloadable extension version");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.113";/, "Manager updater must require the worker version with completed network-stream, renderer wake recovery, and direct plus console/log 429 diagnostics");
