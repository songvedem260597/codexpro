...
assert.equal(manifest.version, "0.5.92", "worker heartbeat must remain packaged with the latest recovery fixes");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.92";/, "Manager updater must require the worker version with the heartbeat fix");
