...
assert.equal(manifest.version, "0.5.123", "prepare injection recovery must be packaged in a reloadable extension version");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.123";/, "Manager updater must require the worker version with prepare injection recovery");
