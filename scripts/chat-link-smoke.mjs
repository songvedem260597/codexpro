...
assert.equal(manifest.version, "0.5.116", "two-tab non-mac cap, one-tab macOS policy, attachment reuse recovery, and prior browser fixes must be packaged in a reloadable extension version");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.116";/, "Manager updater must require the worker version with the latest tab-cap behavior");
