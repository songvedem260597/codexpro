...
assert.equal(manifest.version, "0.5.97", "long-task watchdog must be packaged in a reloadable extension version");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.97";/, "Manager updater must require the worker version with the guarded long-task recovery");
