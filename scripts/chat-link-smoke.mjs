...
assert.equal(manifest.version, "0.5.111", "completed network-stream and renderer wake recovery must be packaged in a reloadable extension version");
assert.match(managerMain, /const WORKER_EXTENSION_VERSION = "0\.5\.111";/, "Manager updater must require the worker version with completed network-stream and renderer wake recovery");
