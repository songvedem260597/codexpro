...
assert.match(manager, /history-rate-limit:[\s\S]*Date\.now\(\) - previous < 120_000[\s\S]*operationsRecoveryTimes\.current\.set\(key, Date\.now\(\)\)[\s\S]*forceContinuation: true/, "history 429 recovery must retain the 120s single-recovery debounce");
assert.match(managerSession, /createCanonicalResponseReadCoordinator[\s\S]*current\.inFlight[\s\S]*current\.nextAllowedAt > currentNow[\s\S]*canonical_poll_deferred/, "Manager canonical callers must share one renderer in-flight and obey cooldown without a real bridge fetch");
